    ...
    public void onAddClick(View view) {
        if (txtPersonName.getText().toString().length() > 0) {
            try {
                ...
                Toast.makeText(NewCelebrationActivity.this, R.string.insertedSuccessful, Toast.LENGTH_SHORT).show();
                MainActivity.countForShowAd++;
                if (MainActivity.countForShowAd > MainActivity.clickCountForShowAd){
        ...
        switch (requestCode) {
            case RequestCodes.PICK_CONTACT:
                if (resultCode == RESULT_OK) {
                    ...
                    Toast.makeText(NewCelebrationActivity.this, getString(R.string.noSelectContact), Toast.LENGTH_SHORT).show();
                }
                break;
        }
    }
